import 'package:flutter/material.dart';
import 'package:material_design_icons_flutter/material_design_icons_flutter.dart';
import 'package:provider/provider.dart';
import '../../../data/dao/local_dao.dart';
import '../../../data/models/local.dart';
import '../../../logic/provider/local_provider.dart';
import '../../widgets/primary_button.dart';

class EditLocalScreen extends StatefulWidget {
  final Local local;
  const EditLocalScreen({super.key, required this.local});

  @override
  State<EditLocalScreen> createState() => _EditLocalScreenState();
}

class _EditLocalScreenState extends State<EditLocalScreen> {
  final _formKey = GlobalKey<FormState>();
  late TextEditingController _nomeController;
  late TextEditingController _areaController;
  late TextEditingController _obsController;
  
  late String _tipo;
  late bool _temQuebraVento;
  late bool _temAreaSensivel;
  bool _isSaving = false;

  final List<String> _tipos = [
    'Campo Aberto',
    'Estufa',
    'Pomar',
    'Hidroponia',
    'Vivero',
    'Outro'
  ];

  @override
  void initState() {
    super.initState();
    _nomeController = TextEditingController(text: widget.local.nome);
    _areaController = TextEditingController(text: widget.local.areaEmMetros.toString());
    _obsController = TextEditingController(text: widget.local.observacoes ?? '');
    _tipo = widget.local.tipo;
    _temQuebraVento = widget.local.quebraVento;
    _temAreaSensivel = widget.local.areaSensivel;
  }

  @override
  void dispose() {
    _nomeController.dispose();
    _areaController.dispose();
    _obsController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Editar Local'),
      ),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(24.0),
        child: Form(
          key: _formKey,
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              const Text(
                'Identificação',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green),
              ),
              const SizedBox(height: 16),
              TextFormField(
                controller: _nomeController,
                decoration: const InputDecoration(
                  labelText: 'Nome do Local *',
                  prefixIcon: Icon(Icons.drive_file_rename_outline),
                ),
                validator: (v) => v == null || v.isEmpty ? '* Obrigatório' : null,
              ),
              const SizedBox(height: 16),
              DropdownButtonFormField<String>(
                value: _tipo,
                decoration: const InputDecoration(
                  labelText: 'Tipo de Local',
                  prefixIcon: Icon(Icons.category_outlined),
                ),
                items: _tipos.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
                onChanged: (v) => setState(() => _tipo = v!),
              ),
              const SizedBox(height: 24),
              const Text(
                'Dimensões e Características',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green),
              ),
              const SizedBox(height: 16),
              TextFormField(
                controller: _areaController,
                keyboardType: TextInputType.number,
                decoration: const InputDecoration(
                  labelText: 'Área em m² *',
                  prefixIcon: Icon(Icons.square_foot),
                ),
                validator: (v) {
                  if (v == null || v.isEmpty) return '* Obrigatório';
                  if (double.tryParse(v) == null) return 'Valor inválido';
                  return null;
                },
              ),
              const SizedBox(height: 16),
              SwitchListTile(
                title: const Text('Tem Quebra-vento?'),
                subtitle: const Text('Proteção contra ventos fortes'),
                secondary: Icon(MdiIcons.windPowerOutline),
                value: _temQuebraVento,
                onChanged: (v) => setState(() => _temQuebraVento = v),
              ),
              SwitchListTile(
                title: const Text('Área Sensível?'),
                subtitle: const Text('Ex: Próximo a rios ou casas'),
                secondary: Icon(MdiIcons.alertOutline),
                value: _temAreaSensivel,
                onChanged: (v) => setState(() => _temAreaSensivel = v),
              ),
              const SizedBox(height: 24),
              const Text(
                'Outros',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green),
              ),
              const SizedBox(height: 16),
              TextFormField(
                controller: _obsController,
                maxLines: 3,
                decoration: const InputDecoration(
                  labelText: 'Observações',
                  alignLabelWithHint: true,
                ),
              ),
              const SizedBox(height: 40),
              PrimaryButton(
                label: 'Salvar Alterações',
                isLoading: _isSaving,
                onPressed: _atualizarLocal,
              ),
            ],
          ),
        ),
      ),
    );
  }

  Future<void> _atualizarLocal() async {
    if (_formKey.currentState!.validate()) {
      setState(() => _isSaving = true);
      
      try {
        final updatedLocal = Local(
          id: widget.local.id,
          nome: _nomeController.text.trim(),
          areaEmMetros: double.parse(_areaController.text),
          tipo: _tipo,
          quebraVento: _temQuebraVento,
          areaSensivel: _temAreaSensivel,
          observacoes: _obsController.text.trim(),
          propriedadeId: widget.local.propriedadeId,
        );

        await LocalDAO().updateLocal(updatedLocal);

        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Local atualizado com sucesso!')),
          );
          Navigator.pop(context, updatedLocal);
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Erro ao atualizar local: $e')),
          );
        }
      } finally {
        if (mounted) setState(() => _isSaving = false);
      }
    }
  }
}
